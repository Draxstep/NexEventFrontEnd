...
export default function TicketReceipt({ event, type, qty, uniqueCode }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col sm:flex-row hover:shadow-md transition-shadow relative">
      ...
      {/* Círculos decorativos para el efecto de boleto (arriba y abajo en la división) */}
      ...
      <div className="p-6 flex-1 sm:w-2/3">
        ...
        <div className="space-y-2 mb-6">
          <div className="flex items-center text-gray-600 text-sm">
            ...
            <span>{event?.fecha || 'Fecha por confirmar'} • {event?.hora || '00:00'}</span>
          </div>
          <div className="flex items-center text-gray-600 text-sm">
            <MapPin size={16} className="mr-2 text-blue-500" />
            <span className="truncate">{event?.lugar || 'Lugar por confirmar'}</span>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-xl p-4 flex justify-between items-center border border-gray-100">
          ...
          <div className="text-right">
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wider mb-1">Cantidad</p>
      ...
      {/* Sección Derecha: Código y "Desprendible" */}
      <div className="border-t-2 sm:border-t-0 sm:border-l-2 border-dashed border-gray-200 bg-blue-600 text-white p-6 sm:w-1/3 flex flex-col items-center justify-center relative">
        ...
        <div className="bg-white p-3 rounded-xl mb-3 shadow-inner">
          {/* Usamos un icono de QR como representación gráfica */}
          <QrCode size={64} className="text-gray-900" />
        </div>
        
        <p className="font-mono text-sm tracking-widest font-bold text-center bg-blue-700/50 px-3 py-1 rounded-lg">
          {uniqueCode || 'TKT-00000000'}
        </p>
      </div>
    </div>
  );
