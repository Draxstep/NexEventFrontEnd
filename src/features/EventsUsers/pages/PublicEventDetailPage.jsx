...
export default function PublicEventDetailPage() {
  ...
  const { fetchEventoById, checkInteresPrevio, registrarInteres, loading, error } = useEventsUsers();
  ...
  const renderToast = () => {
    ...
    return (
      <div className={`fixed bottom-4 right-4 sm:bottom-8 sm:right-8 text-white px-5 py-4 rounded-xl shadow-2xl flex items-center animate-fade-in z-[200] max-w-[90vw] sm:max-w-md border border-white/20 backdrop-blur-sm ${toast.type === 'error' ? 'bg-red-600/95' : 'bg-gray-800/95'}`}>
        {toast.type === 'error' ? <AlertCircle size={24} className="mr-3 flex-shrink-0" /> : <CheckCircle2 size={24} className="mr-3 flex-shrink-0 text-green-400" />}
    ...
    return (
      <div className="w-full max-w-3xl mx-auto mt-12 px-4">
        <div className="bg-red-50 border border-red-200 p-8 rounded-xl text-center text-red-700 animate-fade-in">
          ...
          <p className="text-lg mb-6">{error || 'El evento que buscas no existe.'}</p>
          <button onClick={() => navigate('/eventos')} className="bg-red-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-red-700 transition-colors">
